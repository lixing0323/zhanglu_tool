<template>
  <div>
    <el-card>
      <ht-action-panel>
        <template slot="left">
          <el-input v-model="listQuery.name" placeholder="请输入归类名称" class="filter-item" @keyup.enter.native="onSearch" />
    ...
    <ht-custom-dialog :title="`${isEdit?'编辑':'新建'}`" :visible.sync="dialogVisible" width="500px">
...
export default {
  ...
  methods: {
    ...
    onExport() {
      this.$message({ message: `数据导出成功`, duration: 3000, type: 'success' })
    }
  }
}
</script>
